...
def process_paper_async(paper_id, settings_dict):
    thread = threading.Thread(target=process_paper, args=(paper_id,), kwargs={"settings_dict": settings_dict})
    thread.start()
...
def dashboard_view(request):
    ...
    if request.method == "POST" and "upload_file" in request.POST:
        ...
        if form.is_valid():
            ...
            try:
                # Run processing in background, do NOT block the request
                ...
                # Keep the paper but show error on dashboard
...
def loading_page(request, paper_id):
    ...
    if paper.citations.exists():
        return redirect("viewer_detail", paper_id=paper.id)
