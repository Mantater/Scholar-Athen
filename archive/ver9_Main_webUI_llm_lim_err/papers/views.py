...
def process_paper_async(paper_id, settings_dict):
    def wrapper():
        try:
            ...
            # Delete paper so it doesn't appear in dashboard
            paper = UploadedPaper.objects.filter(id=paper_id).first()
            if paper:
                paper.delete()
    ...
    if request.method == "POST" and "upload_file" in request.POST:
        ...
        if form.is_valid():
            ...
            try:
                ...
                # If upload fails immediately, delete paper and show error
                paper.delete()
...
def loading_page(request, paper_id):
    ...
    Polls asynchronously until citations are ready.
    """
    paper = get_object_or_404(UploadedPaper, id=paper_id)

    # If citations ready or error exists, redirect to viewer
    if paper.citations.exists() or getattr(paper, "error_message", None):
        params = {}
